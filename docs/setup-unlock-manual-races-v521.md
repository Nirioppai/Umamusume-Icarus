...
SweepyModv5.21 fixes two lifecycle/race-planning bugs found after completed Trackblazer careers.
...
Before a fresh run starts, if Manual Selection mode has staged races, SweepyMod automatically saves those races and sends them with the start request as `manual_race_ids` with `race_planner_mode = manual`.
