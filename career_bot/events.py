...
class EventManager:
    def __init__(self, base_dir):
        ...
        self._skill_name_to_ids = {}
        self._skill_id_to_name = {}
        self._planned_cache = (None, set())
    ...
    def _load(self):
        # Event-effect DB sourced from gametora.com + the game's master.mdb,
        # keyed by name slug, each choice carrying a canonical effect string
        # ("Stamina +6, Skill hint +1") that _parse_effect_string scores. Used
        # ONLY as a fallback when the live event payload carries no inline reward
        # for a choice, so existing tuned behavior is unchanged for known events.
        # Generated by tools/gametora_event_scraper.py (skill ids -> names via
        # master.mdb). Falls back to the legacy game8 filename if present.
        scraped_path = self.base_dir / "data" / "event_effects.json"
        if not scraped_path.exists():
            scraped_path = self.base_dir / "data" / "event_effects_game8.json"
        ...
        # skill name<->id maps (for resolving the user's planned skills, so an
        # event choice that hints a planned skill can be preferred). Same source
        # the skill buyer uses, so the picker and buyer agree on identities.
        try:
            sp = self.base_dir / "data" / "skill_data.json"
            if sp.exists():
                sd = json.loads(sp.read_text(encoding="utf-8")) or {}
                for sid, info in (sd.items() if isinstance(sd, dict) else []):
                    try:
                        sid_i = int(sid)
                    except (TypeError, ValueError):
                        continue
                    name = (info.get("name") if isinstance(info, dict) else info) or ""
                    k = self._norm_skill(name)
                    if not k:
                        continue
                    self._skill_name_to_ids.setdefault(k, set()).add(sid_i)
                    self._skill_id_to_name.setdefault(sid_i, str(name))
        except Exception:
            self._skill_name_to_ids = {}
            self._skill_id_to_name = {}
    ...
    def _planned_skill_ids(self, preset):
        """Skill ids the user EXPLICITLY planned to buy: skill_strategy.forced_skills
        + manual_skill_tiers values, minus the blacklist. Names resolved to ids via
        skill_data.json; bare ids accepted. Cached by the (names, blacklist) signature."""
        preset = preset or {}
        strategy = preset.get("skill_strategy") if isinstance(preset.get("skill_strategy"), dict) else {}
        names = list(strategy.get("forced_skills") or [])
        tiers = preset.get("manual_skill_tiers")
        if isinstance(tiers, dict):
            for v in tiers.values():
                if isinstance(v, list):
                    names.extend(v)
        blacklist = list(preset.get("learn_skill_blacklist") or [])
        blacklist.extend(strategy.get("blacklist") or [])
        sig = (tuple(str(x) for x in names), tuple(str(x) for x in blacklist))
        if self._planned_cache[0] == sig:
        ...
        for b in blacklist:
            ...
            if s.isdigit():
                bl_ids.add(int(s))
            bl_names.add(self._norm_skill(s))
        ids = set()
        for n in names:
            ...
            if s.isdigit():
                ids.add(int(s))
                continue
            k = self._norm_skill(s)
            if k in bl_names:
                continue
            ids |= self._skill_name_to_ids.get(k, set())
        ids -= bl_ids
        self._planned_cache = (sig, ids)
        return ids
    ...
    def _score_choices(self, event, choices, preset, chara, current_turn):
        # Effect sources, in precedence: inline reward fields carried in the live
        # event payload, then the gametora+master.mdb scraped-effect DB (so every
        # event scores). The scrape also supplies the per-choice hinted skill ids
        # (discarded by the effect string) used for planned-skill preference.
        ...
        for i, choice in enumerate(choices):
            ...
            reward_detail = None
            if i < len(inline_rewards):
                reward_detail = inline_rewards[i]
            if reward_detail is None:
                reward_detail = scraped_rewards.get(select_index)
            # Always attach the scrape's hinted skill ids (the live inline payload
            # doesn't carry them), so a planned-skill hint can be preferred even
            # when inline rewards drive the rest of the score.
            scraped = scraped_rewards.get(select_index)
            if isinstance(scraped, dict) and scraped.get("hint_skill_ids"):
                reward_detail = dict(reward_detail or {})
                reward_detail.setdefault("hint_skill_ids", scraped["hint_skill_ids"])
            score, reason = self._score_outcome(None, reward_detail, preset, chara, current_turn)
            scored.append({
                "index": i,
    ...
    # ---- game8 scraped-effect fallback (MANT + Support Card events) ------
    _EFFECT_STAT_MAP = {
    ...
    def _scraped_choices_for_event(self, event):
        ...
        for sel, info in choices.items():
            if isinstance(info, dict):
                parsed = self._parse_effect_string(info.get("effect")) or {}
                # Recover the hinted skill identity discarded by the effect string:
                # the scrape's raw array carries {t:"sk", d:skill_id} per hint.
                hint_ids = [r.get("d") for r in (info.get("raw") or [])
                            if isinstance(r, dict) and r.get("t") == "sk" and isinstance(r.get("d"), int)]
                if hint_ids:
                    parsed = dict(parsed)
                    parsed["hint_skill_ids"] = hint_ids
                if parsed:
                    out[str(sel)] = parsed
        return out
    ...
    def _score_reward_detail(self, reward, preset, chara, reason, turn=0):
        ...
        # Common scalar reward fields seen in native payloads or game8 scraped
        # data. Keep these additive so a reward can mix parameter arrays with
        # convenient scalar labels.
        ...
        # Planned-skill preference: if this choice hints a skill the user EXPLICITLY
        # planned to buy, add a moderate bonus on top of the generic hint bonus.
        hint_ids = reward.get("hint_skill_ids")
        if hint_ids:
            planned = self._planned_skill_ids(preset)
            matched = [sid for sid in hint_ids if sid in planned]
            if matched:
                score += tb_rules.EVENT_PLANNED_SKILL_HINT_BONUS * len(matched)
                names = ",".join(self._skill_id_to_name.get(sid, str(sid)) for sid in matched)
                reason.append("skill_hint_planned:" + names if names else "skill_hint_planned")
