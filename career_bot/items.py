...
class MantItemManager:
    ...
    def _hammer_target_for_race(self, owned, turn, grade, cfg, is_climax=False):
        ...
        if grade == "G1":
            if artisan_qty > 0:
                return "Artisan Cleat Hammer"
            # FORK: (nirio) dynamic reserve integrates with upstream's total check.
            if total > finale_reserve and master_qty > 0 and master_qty > protected_mch:
                return "Master Cleat Hammer"
            return None
