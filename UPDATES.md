...
### 2026-06-30 — Log viewer: fix instant-use item leftover display in AI summary
**Commit:** (pending) **File(s):** `log_viewer.html`
**Context:** The AI summary per-item table reported ledger-derived leftover counts for instant-use items (scrolls, manuals, carrots, Training Applications, etc.). These items are consumed on the same turn they are bought, so the ledger (bought − used) always drifts when buy and use events land on the same turn. This made it appear as if scrolls/manuals were left unused (~165 stats lost), when in reality final_inventory at T77 correctly showed 0 for all instant-use items. The fix: for instant-use items, use final_inventory count (ground truth) instead of ledger count in the per-item table. The accounting-mismatch warning was also refined to compare only non-instant-use items, eliminating false mismatch alerts from expected instant-use drift.
**Status:** ACTIVE

### 2026-06-30 — Support-card-driven ankle weight buy prioritization
**Commit:** (pending) **File(s):** `career_bot/items.py`
**Context:** `_skip_buy()` blocked ankle weight purchases when the matching stat had fewer than 2 support cards in the deck. This was too strict: a stat with exactly 1 support card still trains that stat, making its ankle weight useful. Worse, 0-card stats (e.g. Guts with no Guts support cards) were correctly skipped but couldn't be distinguished from 1-card stats. The fix: lower `skip_low_deck` threshold from `< 2` to `< 1` — skip only when the deck has 0 cards of that type. The existing `_item_buy_value` scaling (deck_count * bonus) already ranks anklet types proportionally so higher-count stats get their anklet first; the per-card bonus was also raised from 12 → 20 to make deck composition more decisive when multiple anklet types appear in the shop simultaneously. Net effect: with a 3 Speed / 2 Power / 1 Stamina / 0 Guts deck, Speed Ankle Weights buy first (+60 value), then Power (+40), then Stamina (+20), Guts anklets skipped (0 cards).
**Status:** ACTIVE

### 2026-06-30 — Fix megaphone stacking bug + bootcamp reserve + Coaching opt-in
**Commit:** (pending) **File(s):** `career_bot/items.py`, `career_bot/trackblazer_rules.py`, `public-v3/modals.js`
**Context:** Three related megaphone issues found from run evidence:
1. **Stacking bug:** The bot queued a second megaphone (Empowering at T38) while a Motivating was already active from T37. The game UI disables all megaphone buttons while one is active — the second use call is accepted by the API but the effect is wasted. Fixed by returning None from `_megaphone_target()` immediately when `_active_megaphone_tier(state) > 0`. This is a critical waste prevention.
2. **Bootcamp reserve:** Strong megaphones were being spent on mid-career training turns before both bootcamp windows (T37-40, T61-64), arriving at bootcamp with fewer than the ideal 2 Empowering megas. Added a two-window reserve: hold `nirio_bootcamp_mega_target` (default 2) strong megaphones before classic camp (T37) AND before senior camp (T61). The reserve lifts on entering each bootcamp window. `megaphone_summer_first_turn` corrected from 35 to 37 (first actual training turn, not the pre-camp race turn). Integrated as a FORK: (nirio) extension of the existing `hold_summer_strong` mechanism.
3. **Coaching Megaphone opt-in:** Replaced the upstream `trackblazer_buy_small_megaphone` flag with a nirio UI knob `nirio_buy_coaching_mega` (default False). Both flags checked for backward compatibility. Added UI slider for all 3 new knobs: Bootcamp Mega Target, Senior Bootcamp Turn, Buy Coaching Megaphone.
**Status:** ACTIVE

### 2026-06-30 — Fix log viewer JS syntax error and bootcamp turn null-crash
**Commit:** (pending) **File(s):** `log_viewer.html`
**Context:** Two bugs introduced in `d6f4622` broke log_viewer.html entirely. (1) **Critical syntax error:** The ternary expression `(parts.length?' ('+parts.join(', ')+')')` in `nirioEffectiveness()` was missing the else branch — no `:''` before the closing `)`. JavaScript syntax errors prevent the entire `<script>` block from loading, so clicking "Open Log" / drag-drop both silently failed (no JS functions were defined). (2) **Runtime crash:** When bootcamp turn 37 or 61 was missing from the log (e.g. early-stopped runs), `classicInv`/`seniorInv` were set to `[null]` (array with one null element) instead of `null`, causing `null.item_id` TypeError inside `countStrong()` when copying the AI summary. Fixed both: added `:''` else branch to the ternary; changed `[null]` to `null` so `countStrong(null)` correctly returns `null` and the check is skipped.
**Status:** ACTIVE
